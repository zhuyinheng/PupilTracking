...
static bool is_good_ellipse_evaluation(cv::RotatedRect *ellipse, cv::Mat *pic, int validity_th){
...
static cv::RotatedRect blob_finder(cv::Mat *pic,int validity_th){
...
if(pos.y>0 && pos.y<pic->rows && pos.x>0 && pos.x<pic->cols){
	...
	if( pos.y>0 && pos.y<pic->rows && pos.x>0 && pos.x<pic->cols){
		...
		if(!is_good_ellipse_evaluation(&ellipse, pic, validity_th)){
			ellipse.center.x=0;
			ellipse.center.y=0;
			ellipse.angle=0;
