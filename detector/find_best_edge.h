...
static bool is_good_ellipse_eval(cv::RotatedRect *ellipse, cv::Mat *pic, int *erg, int validity_th){
	...
	*erg=val;
	...
	if(val>validity_th) return true;
	else return false;
}
...
static std::vector<std::vector<cv::Point>> get_curves(cv::Mat *pic, cv::Mat *edge, cv::Mat *magni, int start_x, int end_x, int start_y, int end_y, double mean_dist, int inner_color_range, int validity_th){
	...
	for(int iii=0;iii<all_lines.size();iii++) {
		...
		cv::RotatedRect ellipse;
				...
				if(add_curve){
					...
					if(add_curve){
						if(!is_good_ellipse_eval(&ellipse,pic,&ergebniss,validity_th))
							add_curve=false;
					}
...
static cv::RotatedRect find_best_edge(cv::Mat *pic,cv::Mat *edge, cv::Mat *magni, int start_x, int end_x, int start_y, int end_y, double mean_dist, int inner_color_range, int validity_th){
	...
	std::vector<std::vector<cv::Point>> all_curves=get_curves(pic, edge, magni, start_x, end_x, start_y, end_y, mean_dist, inner_color_range,validity_th);
